...
private:
    ...
    double calc_sum(int e1,int e2,int rel)
    {
        ...
        if (L1_flag)
        	for (int ii=0; ii<n; ii++)
            	sum+=fabs(entity_vec[e2][ii]-entity_vec[e1][ii]-relation_vec[rel][ii]);//实体2 - 实体1 - 实体关系
        else
        	for (int ii=0; ii<n; ii++)
    ...
    void gradient(int e1_a,int e2_a,int rel_a,int e1_b,int e2_b,int rel_b)
    {
        for (int ii=0; ii<n; ii++)
        {
            ...
            double x = 2*(entity_vec[e2_a][ii]-entity_vec[e1_a][ii]-relation_vec[rel_a][ii]);//实体2 - 实体1 - 实体关系
            if (L1_flag)
            	if (x>0)
            		x=1;
            	else
