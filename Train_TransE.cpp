...
public:
	...
	void run(int n_in, double rate_in, double margin_in, int method_in) {
		...
		margin = margin_in;
		method = method_in;
		relation_vec.resize(relation_num); //resize是弄成 等长的 一堆0
...
private:
	...
	double rate, margin;
	double belta;
	vector<int> fb_h, fb_l, fb_r; //fb_h和fb_l是两个实体，fb_r是实体关系
		...
		for (int epoch = 0; epoch < nepoch; epoch++) {
			...
			for (int batch = 0; batch < nbatches; batch++) {
				...
				for (int k = 0; k < batchsize; k++) {
					...
					double pr = 1000 * right_num[fb_r[i]]
							/ (right_num[fb_r[i]] + left_num[fb_r[i]]);
					if (method == 0)
						pr = 500;
					if (rand() % 1000 < pr) {
						while (ok[make_pair(fb_h[i], fb_r[i])].count(j) > 0) //应该是找到 有缺失的 三元组，跳出while
							j = rand_max(entity_num); //作为负例
						train_kb(fb_h[i], fb_l[i], fb_r[i], fb_h[i], j,
								fb_r[i]);
					} else {
						while (ok[make_pair(j, fb_r[i])].count(fb_l[i]) > 0)
							j = rand_max(entity_num);
						train_kb(fb_h[i], fb_l[i], fb_r[i], j, fb_l[i],
								fb_r[i]);
					}
	...
	double calc_sum(int e1, int e2, int rel) {
		...
		if (L1_flag)
			for (int ii = 0; ii < n; ii++)
				sum += fabs(
						entity_vec[e2][ii] - entity_vec[e1][ii]
								- relation_vec[rel][ii]); //实体2 - 实体1 - 实体关系
		else
			for (int ii = 0; ii < n; ii++)
				sum += sqr(
						entity_vec[e2][ii] - entity_vec[e1][ii]
	...
	void gradient(int e1_a, int e2_a, int rel_a, int e1_b, int e2_b,
			int rel_b) {
		for (int ii = 0; ii < n; ii++) {

			double x = 2
					* (entity_vec[e2_a][ii] - entity_vec[e1_a][ii]
							- relation_vec[rel_a][ii]); //实体2 - 实体1 - 实体关系
			if (L1_flag)
				if (x > 0)
					x = 1;
				else
					x = -1;
			relation_tmp[rel_a][ii] -= -1 * rate * x; //错误变小
			entity_tmp[e1_a][ii] -= -1 * rate * x;
			entity_tmp[e2_a][ii] += -1 * rate * x;
			x = 2
					* (entity_vec[e2_b][ii] - entity_vec[e1_b][ii]
							- relation_vec[rel_b][ii]);
			if (L1_flag)
				if (x > 0)
					x = 1;
				else
					x = -1;
			relation_tmp[rel_b][ii] -= rate * x; //错误变大
			entity_tmp[e1_b][ii] -= rate * x;
			entity_tmp[e2_b][ii] += rate * x;
		}
	}
	void train_kb(int e1_a, int e2_a, int rel_a, int e1_b, int e2_b,
			int rel_b) {
		double sum1 = calc_sum(e1_a, e2_a, rel_a); //完整的三元组的
		double sum2 = calc_sum(e1_b, e2_b, rel_b); //随机的一个实体凑的三元组
		if (sum1 + margin > sum2) {
...
void prepare() {
	...
	while (fscanf(f_kb, "%s", buf) == 1) {
		...
		if (relation2id.count(s3) == 0) //使用count，返回的是被查找元素的个数。如果有，返回1；否则，返回0。注意，map中不存在相同元素，所以返回值只能是1或0。
				{
...
int main(int argc, char**argv) {
	...
	int method = 1;
	int n = 100; //embedding的维数
	double rate = 0.001;
	double margin = 1; //论文里的γ
	int i;
